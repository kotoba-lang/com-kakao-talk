(ns kakao_talk.main
  "Kotodama WASM entrypoint for the Kakao_talk clean-room actor (L4) — Clojure port.
  ...
  facts (`kakao_talk.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "kakao_talk")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"kakao_talk.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "kakao_talk-compat" :tier tier :entities entities} 200])
...
(defn start! [] :kakao_talk-compat/ready)
